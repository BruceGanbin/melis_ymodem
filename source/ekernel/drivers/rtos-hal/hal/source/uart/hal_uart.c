...
int rx_buf_len = 4096;
struct sunxi_dma_chan *rx_dma = NULL;
#else
void hal_uart_dma_start(uart_port_t uart_port){};
void hal_uart_dma_stop(void){};
...
#ifdef CONFIG_UART_DMA
int rx_dma_get_drqsrc(int uart_port)
{
	int drqsrc = 1;

	switch (uart_port) {
		case 0:
			drqsrc = DRQSRC_UART0_RX;
			break;
		case 1:
			drqsrc = DRQSRC_UART1_RX;
			break;
		case 2:
			drqsrc = DRQSRC_UART2_RX;
			break;
// extend here with different board config
		/*
		case 3:
			drqdst = DRQSRC_UART3_RX;
			break;
		case 4:
			drqdst = DRQSRC_UART4_RX;
			break;
		case 5:
			drqdst = DRQSRC_UART5_RX;
			break;
		default:
			UART_ERR("uart_port invalid!\n");
			break;
		*/
	}
...
int tx_dma_get_drqdst(int uart_port)
{
	int drqdst = 1;

	switch(uart_port){
		case 0:
			drqdst = DRQDST_UART0_TX;
			break;
		case 1:
			drqdst = DRQDST_UART1_TX;
			break;
		case 2:
			drqdst = DRQDST_UART2_TX;
			break;
		// extend here with different board config
		/*
		case 3:
			drqdst = DRQDST_UART3_TX;
			break;
		case 4:
			drqdst = DRQDST_UART4_TX;
			break;
		case 5:
			drqdst = DRQDST_UART5_TX;
			break;
		default:
			UART_ERR("uart_port invalid!\n");
			break;
...
{
	UART_INFO("rx_dma finished, callback to do something...\n");
}
...
hal_uart_status_t hal_uart_dma_start(uart_port_t uart_port)
{
	...
	rx_buf = hal_malloc_coherent(rx_buf_len);
	if (!rx_buf)
	...
	hal_dcache_clean_invalidate((unsigned long)rx_buf, rx_buf_len);

	/* request dma chan */
	ret = hal_dma_chan_request(&rx_dma);
	...
	ret = hal_dma_callback_install(rx_dma, rx_uart_dma_callback, rx_dma);
	if (ret != HAL_DMA_STATUS_OK) {
		UART_ERR("[uart_dma_start] register dma callback failed!");
		goto ERR;
	}

	rx_drqsrc = rx_dma_get_drqsrc((int)uart_port);

	memset(&rx_slave_config, 0, sizeof(rx_slave_config));

	rx_slave_config.direction = DMA_DEV_TO_MEM;
	rx_slave_config.src_addr = (unsigned long)(uart_base + UART_RBR);
	rx_slave_config.dst_addr = (long unsigned int)rx_buf;
	rx_slave_config.dst_addr_width = DMA_SLAVE_BUSWIDTH_1_BYTE;
	rx_slave_config.src_addr_width = DMA_SLAVE_BUSWIDTH_1_BYTE;
	rx_slave_config.dst_maxburst = DMA_SLAVE_BURST_1;
	rx_slave_config.src_maxburst = DMA_SLAVE_BURST_1;
	rx_slave_config.slave_id = sunxi_slave_id(DRQDST_SDRAM, rx_drqsrc);

	ret = hal_dma_slave_config(rx_dma, &rx_slave_config);
	if (ret != HAL_DMA_STATUS_OK) {
		UART_ERR("[uart_dma_start] uart_dma config error, ret:%d\n", ret);
		goto ERR;
	}

	ret = hal_dma_prep_cyclic(rx_dma, rx_slave_config.dst_addr,rx_buf_len, 1, rx_slave_config.direction);
...
ERR:
	UART_INFO("[uart_dma_start]recv_dma init failed!\n");
...
{
	int ret, tx_drqdst;
	struct sunxi_dma_chan *tx_dma = NULL;
	...
	tx_buf = hal_malloc_coherent(tx_buf_len);
	if (!tx_buf)
		UART_ERR("hal_malloc_coherent tx_buf_len failed!\n");

	memset(tx_buf, 0, tx_buf_len);
	memcpy(tx_buf, temp_buf, tx_buf_len);

	UART_INFO("temp_buf:0x%x tx_buf:0x%x \n", (unsigned long)temp_buf, (unsigned long)tx_buf);

	hal_dcache_clean_invalidate((unsigned long)tx_buf, tx_buf_len);

	/* request dma chan */
	ret = hal_dma_chan_request(&tx_dma);
	if (ret == HAL_DMA_CHAN_STATUS_BUSY) {
	...
	/* register dma callback */
	ret = hal_dma_callback_install(tx_dma, tx_uart_dma_callback, tx_dma);
	if (ret != HAL_DMA_STATUS_OK) {
		UART_ERR("[uart_dma_tx]register dma callback failed!");
		goto ERR;
	}

	tx_drqdst = tx_dma_get_drqdst((int)uart_port);

	memset(&tx_slave_config, 0, sizeof(tx_slave_config));
	tx_slave_config.direction = DMA_MEM_TO_DEV;
	tx_slave_config.dst_addr = (unsigned long)(uart_base + UART_THR);
	tx_slave_config.src_addr = (long unsigned int)tx_buf;
	tx_slave_config.dst_addr_width = DMA_SLAVE_BUSWIDTH_1_BYTE;
	tx_slave_config.src_addr_width = DMA_SLAVE_BUSWIDTH_1_BYTE;
	tx_slave_config.dst_maxburst = DMA_SLAVE_BURST_1;
	tx_slave_config.src_maxburst = DMA_SLAVE_BURST_1;
	tx_slave_config.slave_id = sunxi_slave_id(tx_drqdst, DRQSRC_SDRAM);

	ret = hal_dma_slave_config(tx_dma, &tx_slave_config);
	if (ret != HAL_DMA_STATUS_OK) {
		UART_ERR("[uart_dma_tx]uart_dma config error, ret:%d\n", ret);
		goto ERR;
	}

	ret = hal_dma_prep_device(tx_dma, tx_slave_config.dst_addr, tx_slave_config.src_addr, tx_buf_len, tx_slave_config.direction);
	...
	ret = hal_dma_start(tx_dma);
	if (ret != HAL_DMA_STATUS_OK) {
		UART_ERR("[uart_dma_tx]dma start error, ret:%d\n", ret);
		goto ERR;
	}

	while (hal_dma_tx_status(tx_dma, &size)!= 0) {
		UART_INFO("left_size: %d\n", size);
	}

	ret = hal_dma_stop(tx_dma);
	if (ret != HAL_DMA_STATUS_OK) {
		UART_ERR("[uart_dma_tx]dma stop error, ret:%d\n", ret);
		goto ERR;
	}

	ret = hal_dma_chan_free(tx_dma);
	if (ret != HAL_DMA_STATUS_OK) {
		UART_ERR("[uart_dma_tx]dma free error, ret:%d\n", ret);
		goto ERR;
	}

	hal_free_coherent(tx_buf);
	UART_INFO("hal_free_coherent tx_buf!\n");
...
hal_uart_status_t hal_uart_dma_recv(uart_port_t uart_port, uint8_t *usr_buf, uint32_t usr_buf_len)
{
	int len1, len2;
	...
	hal_dcache_invalidate((unsigned long)rx_buf, rx_buf_len);

	UART_INFO("usr_buf:0x%x rx_buf:0x%x \n", (unsigned long)usr_buf, (unsigned long)rx_buf);
#ifdef UART_LOG_DEBUG
	int i;
	uint8_t *temp = rx_buf;

	UART_INFO("rx_buf:");
	for (i = 0; i < usr_buf_len; i++){
		if (i % 16 == 0)
			UART_INFO("\n");
		UART_INFO("%02x ", temp[i]);
	}
	UART_INFO("\n");
#endif
	/* set and use cur_buf point as the correct start address for memcpy */
	if (cur_buf + usr_buf_len > rx_buf + rx_buf_len) {
		len1 = rx_buf + rx_buf_len - cur_buf;
		len2 = usr_buf_len - len1;

		memcpy(usr_buf, cur_buf, len1);
		usr_buf = usr_buf + len1;
		memcpy(usr_buf, rx_buf, len2);

		cur_buf = rx_buf + len2;
	} else {
		memcpy(usr_buf, cur_buf, usr_buf_len);

		cur_buf = cur_buf + usr_buf_len;
	}

	return 0;
}

hal_uart_status_t hal_uart_dma_stop(void)
{
	int ret;

	ret = hal_dma_stop(rx_dma);
	if (ret != HAL_DMA_STATUS_OK) {
		UART_ERR("[uart_dma_stop] dma stop error, ret:%d", ret);
		goto ERR;
	}

	ret = hal_dma_chan_free(rx_dma);
	if (ret != HAL_DMA_STATUS_OK) {
		UART_ERR("[uart_dma_stop] dma free error, ret:%d", ret);
		goto ERR;
	}

	hal_free_coherent(rx_buf);
	UART_INFO("hal_free_coherent rx_buf!\n");
	rx_buf = NULL;
...
static uint32_t uart_handle_rx(uart_port_t uart_port, uint32_t lsr)
{
	...
	uart_priv_t *uart_priv = &g_uart_priv[uart_port];
	if(uart_priv->func) {
		uart_priv->func(UART_EVENT_RX_COMPLETE, uart_priv);
	}
	return lsr;
}
...
int32_t hal_uart_init(int32_t uart_port)
{
	...
	if (uart_mailbox[uart_port] == NULL)
		uart_mailbox[uart_port] = hal_mailbox_create(uart_name, 1024 * 2); // soft queue max to 1k
...
#if defined(CONFIG_UART_DMA) && defined(CONFIG_DRIVER_SYSCONFIG)
	int val;
	char uart_dma_name[15] = {0};

	sprintf(uart_dma_name, "uart%d_dma_used", (int)uart_port);

	if (hal_cfg_get_keyvalue("uart_dma", uart_dma_name, (int32_t *)&val, 1) == 0) {
		if (val == 1) {
...
void hal_uart_irq_callback(int32_t uart_port, uart_callback_t cb, void* param)
{
	uart_priv_t *uart_priv = &g_uart_priv[uart_port];
	uart_priv->func = cb;
	uart_priv->arg = param;
