...
void melis_fstat_to_libc_stat(__ES_FSTAT *fstat, struct stat *st);
#endif
...
static hal_sem_t sys_time_sem = NULL;
...
static inline int check_melis_filesystem(const char *path)
{
    /* path like `/mnt/test`  */
    if (path[0] == '/')
        return 0;
...
int _fstat_r(struct _reent *ptr, int fd, struct stat *pstat)
{
    ...
    else if (fd < 3 + NR_MAXOPEN)
    {
        ...
        ptr->_errno = 0;
        melis_fstat_to_libc_stat(&fstat, pstat);
        return 0;
    }
#endif
    else
...
{
    if (check_melis_filesystem(name) == 1)
    {
#ifdef CONFIG_MELIS_VIRTUAL_FILESYSTEM
        if (esFSYS_mkdir(name) < 0)
        {
            return -1;
        }
        ptr->_errno = 0;
...
int _open_r(struct _reent *ptr, const char *file, int flags, int mode)
{
    ...
    if (check_melis_filesystem(file) == 1)
    {
#ifdef CONFIG_MELIS_VIRTUAL_FILESYSTEM
        __hdle fp;
        char *vfs_mode;
...
_ssize_t _read_r(struct _reent *ptr, int fd, void *buf, size_t nbytes)
{
    if (fd < 3 || fd == libc_stdio_get_console())
    {
        ...
        if (get_default_console()) {
            ...
            int cnt = 0;
            while(cnt == 0) {
                cnt = cli_console_read(get_clitask_console(), buf, nbytes);
                if(cnt == 0) rt_thread_mdelay(100);
            }
            return cnt;
        } else {
#endif
...
{
    if (check_melis_filesystem(old) == 1)
    {
#ifdef CONFIG_MELIS_VIRTUAL_FILESYSTEM
...
int _gettimeofday_r(struct _reent *ptr, struct timeval *tv, void *__tzp)
{
    ...
    hal_sem_timedwait(sys_time_sem, HAL_WAIT_FOREVER);
...
void *_realloc_r(struct _reent *ptr, void *old, size_t newlen)
{
    ...
    result = (void *)hal_realloc(old, newlen);
    if (result == NULL)
    {
        ptr->_errno = ENOMEM;
    }
...
void *_calloc_r(struct _reent *ptr, size_t size, size_t len)
{
    ...
    result = (void *)hal_calloc(size, len);
    if (result == NULL)
    {
        ptr->_errno = ENOMEM;
    }
...
{
    return hal_malloc_align(__size, __alignment);
}
...
{
    if (check_melis_filesystem(file) == 0) {
        goto dfs_unlink;
    }
...
const unsigned long SWIHandler_entity_NLIBOPS[] =
{
    ...
    (const unsigned long) dup                           ,//dup              ;
    (const unsigned long) dup2                          ,//dup2             ;
    (const unsigned long) access                        ,//access           ;
};
#endif
