    ...
    rt_size_t len)
{
    ...
    uint32_t cwd_strlen = 0;

    getcwd(cctx->fpath, DFS_PATH_MAX-1);
    cwd_strlen = strlen(cctx->fpath);
    cctx->fpath[cwd_strlen] = '/';
    cwd_strlen++;

    rt_strncpy(&(cctx->fpath[cwd_strlen]), (const char *)buf, DFS_PATH_MAX - cwd_strlen);
